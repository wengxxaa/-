...
const int heightoffest=210;		//���̵׵ĸ߶�
const float selectboxsize=0.65f;	//ѡ��ķ�������
...
bool AngleManage::init()
{
	...
	selectboxoffest=GameDataInstance()->BANER_HEIGHT;	//�ɹ�ѡ�񷽿�ĸ߶�
...
void AngleManage::Rotate(int key)	//���ָ���ѡ��box
{
	if(key==1)
	{
		...
		if(!_selectExit[i])
		{
			...
			for(int j=i+1;j<3;j++)
			{
				if(_selectExit[j])
				{
					...
					box->runAction(MoveBy::create(startnexttime,Vec2(-nextboxoffest*(j-i),0)));
					_selectExit[i]=true;
					_selectExit[j]=false;
					box->setTag(1000+i);
					temp=true;
			...
			if(!temp)
			{
				...
				selectBox->runAction(MoveBy::create(startnexttime,Vec2(-WINSIZE.width,0)));
				_selectExit[i]=true;
			}
		}
	}
...
bool AngleManage::onTouchBegan(Touch *touch, Event *unused_event)
{
	...
	{
		if (_touchselect > 0&&getTouchState()==TOUCH_TRUE)
			endTouch();
		return false;
	}
...
void AngleManage::endTouch()
{
	...
	if (fillvec.empty())
	{
		...
		box->runAction(Sequence::create(MoveTo::create(0.1f, box->getCenterAnPointX(Vec2(nextboxbegin + nextboxoffest*(_touchselect - 1), selectboxoffest))), DelayTime::create(0.1f),CallFunc::create([this]() {
			_touchselect = 0;
			setTouchState(TOUCH_TRUE);
		}), nullptr));
	}
...
void AngleManage::showTipBox(AngleNode *box)	//��ʾ��ʾ�ķ���
{
	auto fillvec=getFillBlack(box,box->getPosition());
	if(!fillvec.empty())
	{
...
void AngleManage::removeTipBox()	//�Ƴ���ʾ�ķ���
{
...
bool AngleManage::chessFailedJudge()	//ʧ���ж�
{
	for(int i=0;i<3;i++)
	{
		if(_selectExit[i])
