...
const float cleardelaytime=0.1f;	//
...
void StartManage::startDrop()
{
	...
	{
		if (GameData::getSaveData()->_propnum[GameData::getSaveData()->_hupropid - 1] > 0)
		{
		...
		if(_guidenumber>3)
		{
			...
			if(k==(_guidenumber-3))
			{
				if(_selectprop==0&&_eliminateNum==0)
				{
					...
					this->getChildByName(cjTTFLabel::getNameByInt("prop%d",k))->setLocalZOrder(1);
					if(k==1||k==2||k==4)
					{
...
void StartManage::LevelUpMotion()
{
	if(_guide)
	{
		...
		fnt2->runAction(Sequence::create(MoveBy::create(0.3f,Vec2(0,200)),DelayTime::create(0.2f),ScaleTo::create(0.2f,1.2f),ScaleTo::create(0.2f,1.0f),CCDelayTime::create(1.0f),CCFadeOut::create(0.3f),
...
void StartManage::retainScoreMotion(int number,int score,float ft)
{
	...
	fnt1->runAction(FadeIn::create(0.3f));
	fnt1->runAction(Sequence::create(MoveBy::create(0.2f,Vec2(-WINSIZE.width/2.0f-100,0)),DelayTime::create(ft-0.4f),MoveBy::create(0.2f,Vec2(-WINSIZE.width/2.0f-100,0)),nullptr));
	...
	fnt2->runAction(Sequence::create(MoveBy::create(0.2f,Vec2(-WINSIZE.width/2.0f-100,0)),DelayTime::create(ft-0.4f),MoveBy::create(0.2f,Vec2(-WINSIZE.width/2.0f-100,0)),CallFunc::create([fnt1,fnt2,this](){
		fnt1->removeFromParent();
		fnt2->removeFromParent();
	}),nullptr));
}
...
void StartManage::startGuide()
{
	...
	_huptimer->setLocalZOrder(101);
	auto people=this->getChildByName("people");
	people->setLocalZOrder(101);
	auto par = this->getChildByName("par");
	par->setLocalZOrder(102);
...
void StartManage::setGuide2()
{
	...
	touchListener->onTouchBegan = [=](Touch* touch, Event* event){
		...
		auto people=this->getChildByName("people");
		people->setLocalZOrder(3);
		auto par = this->getChildByName("par");
		par->setLocalZOrder(4);
		_huptimer->setLocalZOrder(2);
		return true;
	};
...
void StartManage::useProp4(pair<int,int> pos)
{
	...
	auto cancel=cjMenu::createWithImage("skill2_back.png",[=](Ref *mu){
		...
		node->setLocalZOrder(CHESSMAN_LEVEL);
		_selectprop=0;
	});
...
void StartManage::hongbaoPage()
{
	...
	if(_guidenumber==3)
	{
		...
		this->getChildByName("hongbaomenu")->setLocalZOrder(1);
		this->removeChildByName("hand");
	}
