...
bool SceneManage::init()
{
	...
	if(tempbool)
	{
		...
		runAction(Sequence::create(DelayTime::create(leveluptime),CallFunc::create([this](){startSaveBox();}),nullptr));
...
bool SceneManage::checkChess()
{
	...
	if(eraselise.size()>0)
	{
		...
		this->runAction(Sequence::create(DelayTime::create(tt),CallFunc::create([this](){
			if(!levelUp())
			{
				runAction(Sequence::create(DelayTime::create(CHESS_REMOVE_TIME),CallFunc::create([this](){
					startDropBox();
				}),nullptr));
			}
		}),nullptr));
	...
	action.pushBack(CallFunc::create([this](){
		for(int i=0;i<CHESS_SIZE_W;i++)
			for(int j=0;j<CHESS_SIZE_H;j++)
			{
				...
				if(_moveh[j]>0&&box)
				{
					auto moveto=MoveTo::create(CHESS_REMOVE_TIME*0.8f,box->getPosition()-Vec2(0,_moveh[j]*CHESS_SIZE_SIZE));
		...
		else
		{
			...
			if(_touchtime<0.3f&&(recentpos.y<previouspos.y)&&(abs(recentpos.x-previouspos.x)<abs(recentpos.y-previouspos.y))&&(previouspos.y-recentpos.y)>=CHESS_SIZE_SIZE)
			{
				...
				if(hoffest>0)
				{
					...
					recentBox->runAction(Sequence::create(easein,CallFunc::create([](){
...
void SceneManage::moveBoxToId(int boxid)
{
	...
	if(ontop)
	{
		...
		auto move1=MoveBy::create(droptime,Vec2(CHESS_SIZE_SIZE*ofpos.first,0));
		auto call=CallFunc::create([recentbox,pointtip](){
			recentbox->setRotate(pointtip.angle);
		});
		auto move2=MoveBy::create(droptime,Vec2(0,ofpos.second*CHESS_SIZE_SIZE));
		auto ease2=EaseIn::create((ActionInterval*)move2,EASE_STRENGTH);
		recentbox->runAction(Sequence::create(move1,call,ease2,CallFunc::create([](){
				cjMusic::playEffect("video/drop-Dekaron.mp3");
		}),DelayTime::create(CHESS_CHECK_TIME),CallFunc::create([recentbox,pointtip,this](){
			if(getGameState()==STATE_STOP)
				setGameState(STATE_RUN);
	...
	else
	{
		...
		auto move1=MoveBy::create(droptime,Vec2(CHESS_SIZE_SIZE*ofpos.first,0));
		auto call=CallFunc::create([recentbox,toptip](){
			recentbox->setRotate(toptip.angle);
		});
		auto move2=MoveBy::create(droptime,Vec2(0,ofpos.second*CHESS_SIZE_SIZE));
		recentbox->runAction(Sequence::create(move1,call,move2,CallFunc::create([recentbox,pointtip,this](){
...
void SceneManage::PointMove(float ft)
{
	...
	if(unsigned(_pointMoveNumber)>=_pointMoveP.size())
	{
		...
		auto ease=EaseIn::create((ActionInterval*)MoveTo::create(1/60.0f,chessPos(ppp.position.first,ppp.position.second)),EASE_STRENGTH);
		box->runAction(ease);

		_recentPos=ppp.position;
		unschedule(schedule_selector(SceneManage::PointMove));
		this->runAction(Sequence::create(DelayTime::create(CHESS_CHECK_TIME),CallFunc::create([this](){
			if(getGameState()==STATE_STOP)
				setGameState(STATE_RUN);
			if(boxSure())
			{
...
void SceneManage::textShow(const string& str,int size,Vec2 pos)
{
	...
	text->runAction(Sequence::create(ScaleTo::create(0.5f,1.1f),ScaleTo::create(0.5f,1.0f),CallFunc::create([text](){
...
void SceneManage::addParticle(int x,int y,int shapeid,int type)
{
	...
	auto par = ParticleSystemQuad::create(parstr.c_str());
...
void SceneManage::spriteShow(const string& str)
{
	...
	spr->runAction(Sequence::create(FadeOut::create(0.08f),FadeIn::create(0.08f),FadeOut::create(0.08f),FadeIn::create(0.08f),FadeOut::create(0.08f),FadeIn::create(0.08f),
...
bool SceneManage::levelUp()
{
	if(getmScore()>=getmTarget())
	{
		...
		runAction(Sequence::create(DelayTime::create(sprintshowtime),CallFunc::create([this](){
			LevelUpEliminate();
		}),nullptr));
		return true;
	}
...
void SceneManage::LevelUpMotion()
{
	...
	fnt1->runAction(Sequence::create(MoveBy::create(0.3f, Vec2(0, 200)), DelayTime::create(1.1f), FadeOut::create(0.3f), nullptr));
...
void SceneManage::Resurrection()
{
	...
	setGameState(STATE_RUN);
	runAction(Sequence::create(DelayTime::create(tt),CallFunc::create([this](){
		if(_model==MODEL_SLIP)
		{
			schedule(schedule_selector(SceneManage::boxDropTip),0.1f);
		}
